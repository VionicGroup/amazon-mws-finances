...
abstract class MWSFinancesService_Model
{
            ...
            if (is_array($fieldType)) {
                if ($fieldType[0] == "object") {
                    ...
                    if ($elements->length >= 1) {
                        $model = 'MWSF\Model\\' . $fieldType[0];
                        foreach ($elements as $element) {
                            $this->_fields[$fieldName]['FieldValue'][] = new $model($element);
                        }
                    } 
                } else {
                    if (isset($field['ListMemberName'])) {
                ...
                if ($this->_isComplexType($fieldType)) {
                    $elements = $xpath->query("./*[local-name()='$fieldName']", $dom);
                    if ($elements->length == 1) {
                        $model = 'MWSF\Model\\' . $fieldType;
                        $this->_fields[$fieldName]['FieldValue'] = new $model($elements->item(0));
                    }   
                } else {
                    if($fieldType[0] == "@") {
            ...
            if (is_array($fieldType)) {
                if ($this->_isComplexType($fieldType[0])) {
                 ...
                 if ($this->_isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        $model = 'MWSF\Model\\' . $fieldType;
                        $this->_fields[$fieldName]['FieldValue'] = new $model($array[$fieldName]);
                    }   
                 } else {
                    if (array_key_exists($fieldName, $array)) {
    ...
    *
    * @param mixed $var
    * @return boolean if passed variable is an numeric array
    */
